...
Override stored credentials with an API token.
...
**Use case:** CI/CD environments or ephemeral containers.
...
Use environment variables instead of stored credentials:
...
2. `~/.config/tuna/` credential files (default)
